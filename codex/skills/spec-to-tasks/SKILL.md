...
- Every task gets a dependency marker: Independent, or Depends on with the IDs it depends on.
- Group by phase/milestone from the spec.
- End with a "Parallel Work Guide" section that summarizes which tasks can be farmed out simultaneously and which chains must run sequentially.
